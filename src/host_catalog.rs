use serde_json::{Value, json};

type ToolSpec = (
    &'static str,
    &'static str,
    &'static [(&'static str, &'static str)],
    &'static [(&'static str, &'static str)],
    bool,
    bool,
);

pub fn append(tools: &mut Vec<Value>) {
    let specs: &[ToolSpec] = &[
        (
            "process_module_dump",
            "Dump only the loaded main EXE image of a Windows process to a rebuilt PE for analysis. Supply pid or an unambiguous process name; unreadable pages require allowPartial=true.",
            &[("outputPath", "string")],
            &[
                ("pid", "integerNumber"),
                ("name", "string"),
                ("overwrite", "boolean"),
                ("allowPartial", "boolean"),
                ("maxImageBytes", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "hardware_inventory",
            "Inventory CPU, GPU adapters, physical RAM modules, drivers, and OS on Windows.",
            &[],
            &[],
            true,
            true,
        ),
        (
            "archive_inspect",
            "List bounded ZIP entries, sizes, CRCs, encryption and unsafe-path warnings without extraction.",
            &[("archive", "string")],
            &[("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "archive_extract_selected",
            "Extract selected ZIP entry names with traversal protection and a 1 GiB output limit.",
            &[
                ("archive", "string"),
                ("destination", "string"),
                ("entries", "string[]"),
            ],
            &[("overwrite", "boolean")],
            false,
            false,
        ),
        (
            "http_download_file",
            "Stream an HTTP(S) response to a temporary file, optionally verify SHA-256, then persist it.",
            &[("url", "string"), ("path", "string")],
            &[
                ("expectedSha256", "string"),
                ("maxBytes", "integerNumber"),
                ("timeoutMs", "integerNumber"),
                ("overwrite", "boolean"),
            ],
            false,
            false,
        ),
        (
            "file_tail",
            "Read recent UTF-8/UTF-16 log lines or new bytes after a cursor, with bounded output.",
            &[("path", "string")],
            &[
                ("cursor", "integerNumber"),
                ("lines", "integerNumber"),
                ("maxBytes", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "file_lock_holders",
            "Find Windows applications and PIDs using a file through Restart Manager.",
            &[("path", "string")],
            &[],
            true,
            true,
        ),
        (
            "file_diff",
            "Compare two files as UTF-8 lines or binary bytes, returning bounded changes and hashes.",
            &[("leftPath", "string"), ("rightPath", "string")],
            &[("mode", "string"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "file_trash",
            "Move a path to the OS Recycle Bin or Trash; permanent=true explicitly deletes it instead.",
            &[("path", "string")],
            &[("permanent", "boolean")],
            false,
            false,
        ),
        (
            "binary_pattern_search",
            "Search a file for hexadecimal bytes with ?? wildcards and return bounded offsets.",
            &[("path", "string"), ("pattern", "string")],
            &[("limit", "integerNumber"), ("startOffset", "integerNumber")],
            true,
            true,
        ),
        (
            "pe_inspect",
            "Inspect an on-disk PE image: architecture, headers, sections, imports, exports, and signature presence.",
            &[("path", "string")],
            &[("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "image_inspect",
            "Inspect image format, dimensions, transparency and bounded EXIF metadata.",
            &[("path", "string")],
            &[],
            true,
            true,
        ),
        (
            "text_transcode",
            "Convert a UTF-8/UTF-16 text file's encoding, BOM and line endings via atomic output.",
            &[
                ("source", "string"),
                ("destination", "string"),
                ("encoding", "string"),
            ],
            &[
                ("lineEndings", "string"),
                ("bom", "boolean"),
                ("overwrite", "boolean"),
            ],
            false,
            false,
        ),
        (
            "file_open",
            "Open an existing file or directory with the Windows shell, or reveal and select it in Explorer. Opening an executable or script may run it.",
            &[("path", "string")],
            &[("mode", "fileOpenMode")],
            false,
            false,
        ),
        (
            "artifact_manifest",
            "Hash a directory into a stable artifact snapshot, optionally saving JSON outside or inside the tree.",
            &[("root", "string")],
            &[
                ("outputPath", "string"),
                ("includeEntries", "boolean"),
                ("limit", "integerNumber"),
                ("overwrite", "boolean"),
            ],
            false,
            false,
        ),
        (
            "manifest_diff",
            "Compare two directories or saved artifact manifest JSON files by relative path, type, size and SHA-256.",
            &[("left", "string"), ("right", "string")],
            &[("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "diagnostics_parse",
            "Normalize common compiler and build diagnostics from text or a log file into file, line, severity, code and message.",
            &[],
            &[
                ("text", "string"),
                ("path", "string"),
                ("limit", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "test_results_parse",
            "Summarize JUnit XML, Visual Studio TRX or TAP test results and return bounded failure details.",
            &[("path", "string")],
            &[("format", "string"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "structured_data_query",
            "Read JSON, TOML, YAML, XML or CSV and select a value using an RFC 6901 JSON Pointer.",
            &[("path", "string")],
            &[("format", "string"), ("pointer", "string")],
            true,
            true,
        ),
        (
            "structured_data_diff",
            "Compare two JSON, TOML, YAML, XML or CSV documents and report bounded pointer-addressed changes.",
            &[("leftPath", "string"), ("rightPath", "string")],
            &[
                ("format", "string"),
                ("pointer", "string"),
                ("limit", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "dependency_inventory",
            "Inventory direct dependencies and available lock or local install versions in Cargo, Python, npm and NuGet projects.",
            &[("root", "string")],
            &[("maxDepth", "integerNumber"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "workspace_scan",
            "Discover repositories and build manifests beneath a directory without entering generated output directories.",
            &[("root", "string")],
            &[("maxDepth", "integerNumber"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "repo_status_batch",
            "Summarize branch, HEAD, changed files and linked worktrees for repositories beneath a directory.",
            &[("root", "string")],
            &[("maxDepth", "integerNumber"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "command_pipeline",
            "Run up to 32 explicit executable steps in order, capturing bounded output and stopping at failure by default.",
            &[("steps", "pipelineSteps")],
            &[("continueOnError", "boolean")],
            false,
            false,
        ),
        (
            "environment_snapshot",
            "Capture OS identity, selected environment variables and bounded executable version probes.",
            &[],
            &[
                ("executables", "string[]"),
                ("variables", "string[]"),
                ("versionArguments", "string[]"),
            ],
            true,
            true,
        ),
        (
            "file_watch",
            "Wait for added, removed or metadata-modified files beneath a directory; return bounded change events.",
            &[("root", "string")],
            &[
                ("recursive", "boolean"),
                ("timeoutMs", "integerNumber"),
                ("intervalMs", "integerNumber"),
                ("limit", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "process_start",
            "Start a process with separate executable and argument array; retain bounded output for process_output.",
            &[("executable", "string")],
            &[
                ("arguments", "string[]"),
                ("cwd", "string"),
                ("env", "stringMap"),
            ],
            false,
            false,
        ),
        (
            "process_run",
            "Run an executable and return exit status and bounded stdout/stderr; time out and kill if requested.",
            &[("executable", "string")],
            &[
                ("arguments", "string[]"),
                ("cwd", "string"),
                ("env", "stringMap"),
                ("timeoutMs", "integerNumber"),
                ("maxOutputBytes", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "process_get",
            "Inspect a PID, including memory, CPU sampled over an interval, command line and I/O counters.",
            &[("pid", "integerNumber")],
            &[("sampleMs", "integerNumber")],
            true,
            true,
        ),
        (
            "process_list",
            "List processes, optionally filtering by case-insensitive name substring.",
            &[],
            &[("name", "string"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "process_tree",
            "Return a process and its descendants from a PID snapshot.",
            &[("pid", "integerNumber")],
            &[],
            true,
            true,
        ),
        (
            "process_wait",
            "Wait for a managed child or for an arbitrary PID to disappear.",
            &[("pid", "integerNumber")],
            &[("timeoutMs", "integerNumber")],
            true,
            true,
        ),
        (
            "process_output",
            "Read bounded stdout/stderr captured from a process started by this server; release after exit to free its slot.",
            &[("pid", "integerNumber")],
            &[
                ("release", "boolean"),
                ("stdoutOffset", "integerNumber"),
                ("stderrOffset", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "process_stop",
            "Stop a PID, optionally including descendants; force defaults to true.",
            &[("pid", "integerNumber")],
            &[
                ("tree", "boolean"),
                ("force", "boolean"),
                ("expectedStartTimeUnix", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "process_modules",
            "List loaded modules of a Windows process.",
            &[("pid", "integerNumber")],
            &[],
            true,
            true,
        ),
        (
            "file_find",
            "Find files by basename wildcard under a root directory.",
            &[("root", "string")],
            &[
                ("pattern", "string"),
                ("recursive", "boolean"),
                ("limit", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "text_search",
            "Search UTF-8 files for literal or regex text and return line/column matches.",
            &[("root", "string"), ("pattern", "string")],
            &[
                ("regex", "boolean"),
                ("caseSensitive", "boolean"),
                ("limit", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "file_stat",
            "Return file type, size and modification time.",
            &[("path", "string")],
            &[],
            true,
            true,
        ),
        (
            "file_hash",
            "Stream a file and compute SHA-256 or SHA-512.",
            &[("path", "string")],
            &[("algorithm", "string")],
            true,
            true,
        ),
        (
            "file_read_range",
            "Read a bounded byte range as UTF-8 or hexadecimal.",
            &[
                ("path", "string"),
                ("offset", "integerNumber"),
                ("length", "integerNumber"),
            ],
            &[("encoding", "string")],
            true,
            true,
        ),
        (
            "file_write_atomic",
            "Write UTF-8 content via a temporary file and atomic persist.",
            &[("path", "string"), ("content", "string")],
            &[("overwrite", "boolean")],
            false,
            false,
        ),
        (
            "file_copy_move",
            "Copy or move a file or directory to a destination.",
            &[
                ("source", "string"),
                ("destination", "string"),
                ("operation", "string"),
            ],
            &[("overwrite", "boolean")],
            false,
            false,
        ),
        (
            "port_owner",
            "Find Windows TCP or UDP endpoints and owning PIDs for a local port.",
            &[("port", "integerNumber")],
            &[("protocol", "string")],
            true,
            true,
        ),
        (
            "service_get",
            "Read Windows service status, startup mode and process ID.",
            &[("name", "string")],
            &[],
            true,
            true,
        ),
        (
            "service_control",
            "Start, stop or restart a Windows service.",
            &[("name", "string"), ("action", "string")],
            &[],
            false,
            false,
        ),
        (
            "eventlog_query",
            "Read recent Windows event log entries.",
            &[("logName", "string")],
            &[("maxEvents", "integerNumber")],
            true,
            true,
        ),
        (
            "registry_read",
            "Read one Windows registry property using a literal registry path.",
            &[("path", "string"), ("property", "string")],
            &[],
            true,
            true,
        ),
        (
            "environment_get",
            "Read one environment variable from process, user or machine scope.",
            &[("name", "string")],
            &[("scope", "string")],
            true,
            true,
        ),
        (
            "system_info",
            "Read OS, CPU and memory information.",
            &[],
            &[],
            true,
            true,
        ),
        (
            "archive_create",
            "Create a ZIP archive from file and directory paths.",
            &[("archive", "string"), ("paths", "string[]")],
            &[("overwrite", "boolean")],
            false,
            false,
        ),
        (
            "archive_extract",
            "Extract a ZIP archive with path traversal protection.",
            &[("archive", "string"), ("destination", "string")],
            &[("overwrite", "boolean")],
            false,
            false,
        ),
        (
            "powershell_run",
            "Run a supplied PowerShell script with timeout and bounded captured output (Windows only).",
            &[("script", "string")],
            &[
                ("timeoutMs", "integerNumber"),
                ("maxOutputBytes", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "process_input",
            "Write UTF-8 or hex bytes to a managed process stdin and optionally close it.",
            &[("pid", "integerNumber")],
            &[
                ("data", "string"),
                ("encoding", "string"),
                ("close", "boolean"),
            ],
            false,
            false,
        ),
        (
            "wait_for",
            "Wait for a file, process exit, TCP port or HTTP status with a bounded timeout.",
            &[("kind", "string")],
            &[
                ("path", "string"),
                ("pid", "integerNumber"),
                ("host", "string"),
                ("port", "integerNumber"),
                ("url", "string"),
                ("expectedStatus", "integerNumber"),
                ("timeoutMs", "integerNumber"),
                ("intervalMs", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "file_patch",
            "Atomically replace exact UTF-8 or hex byte sequences with expected match count and optional SHA-256 precondition.",
            &[
                ("path", "string"),
                ("find", "string"),
                ("replace", "string"),
            ],
            &[
                ("mode", "string"),
                ("expectedMatches", "integerNumber"),
                ("expectedSha256", "string"),
            ],
            false,
            false,
        ),
        (
            "http_request",
            "Send a bounded HTTP or HTTPS request and return status, headers and body.",
            &[("url", "string")],
            &[
                ("method", "string"),
                ("headers", "stringMap"),
                ("body", "string"),
                ("timeoutMs", "integerNumber"),
                ("maxBodyBytes", "integerNumber"),
            ],
            false,
            false,
        ),
        (
            "network_probe",
            "Probe a TCP port or verified TLS handshake and report latency and certificate hash.",
            &[("host", "string"), ("port", "integerNumber")],
            &[("protocol", "string"), ("timeoutMs", "integerNumber")],
            true,
            true,
        ),
        (
            "dns_query",
            "Query A, AAAA, CNAME, MX, NS, TXT or SRV records with a bounded timeout.",
            &[("name", "string")],
            &[("recordType", "string"), ("timeoutMs", "integerNumber")],
            true,
            true,
        ),
        (
            "executable_resolve",
            "Resolve an executable by path or PATH and report its architecture, file size and available version metadata.",
            &[("name", "string")],
            &[],
            true,
            true,
        ),
        (
            "directory_manifest",
            "Inventory a directory with optional SHA-256 hashes and a bounded entry count.",
            &[("root", "string")],
            &[("hashFiles", "boolean"), ("limit", "integerNumber")],
            true,
            true,
        ),
        (
            "file_signature",
            "Inspect Authenticode status, signer and version information for a Windows file.",
            &[("path", "string")],
            &[],
            true,
            true,
        ),
        (
            "scheduled_task",
            "List, inspect, run, stop, enable, disable, register or unregister Windows scheduled tasks.",
            &[("operation", "string")],
            &[
                ("name", "string"),
                ("taskPath", "string"),
                ("limit", "integerNumber"),
                ("executable", "string"),
                ("actionArguments", "string"),
                ("trigger", "string"),
                ("at", "string"),
                ("overwrite", "boolean"),
            ],
            false,
            false,
        ),
        (
            "eventlog_follow",
            "Read Windows event log entries after a record ID and return the next cursor.",
            &[("logName", "string")],
            &[
                ("afterRecordId", "integerNumber"),
                ("maxEvents", "integerNumber"),
            ],
            true,
            true,
        ),
        (
            "performance_sample",
            "Sample CPU, memory and network transfer over an interval; include disk capacity.",
            &[],
            &[("sampleMs", "integerNumber")],
            true,
            true,
        ),
        (
            "acl_get",
            "Read a Windows file or directory owner, SDDL and access entries.",
            &[("path", "string")],
            &[],
            true,
            true,
        ),
    ];
    for &(name, description, required, optional, read_only, idempotent) in specs {
        crate::catalog::add_extra(tools, name, description, required, optional);
        let may_execute = matches!(name, "environment_snapshot");
        tools.last_mut().unwrap()["annotations"] = json!({
            "readOnlyHint": read_only && !may_execute,
            "destructiveHint": !read_only || may_execute,
            "idempotentHint": idempotent,
            "openWorldHint": true
        });
    }
}
